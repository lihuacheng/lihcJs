...
var import_backCall = function(page,page_size){
	import_filter.page = parseInt(page);
	import_filter.page_size = parseInt(page_size);
	var offset = import_filter.page;//(filter.page-1);//*
	var limit = import_filter.page_size;
	...
	for(var n=0;n<list.length;n++){
        ...
        for(var i=0;i<goods.length;i++){
            ...
            if(i==0){
				...
				//拆单
				if(split_able){
					goodsHtml += '<div><a href="'+$.homeU("Index/splitSalesOrder", {'sales_order_id':product.sales_order_id})+'" class="J_split_sales" data-order-id="'+product.sales_order_id+'">拆单</a></div>';
				}
	            goodsHtml+='</td>';
            }
            goodsHtml += '</tr>';
...
var operaType = $.getQueryString("operaType");
if($.string.isEmpty(operaType))operation_type.val("add");
else{
	operation_type.val(operaType)
}
var orderid = $.getQueryString("orderid")
if($.string.isNotEmpty(orderid)&&operaType=="edit"){
	getSalesOrder(orderid,"edit");
}